      ...
      .tween('coloring', function (d) {
        ...
        if (d.end.transition == Change.Delete) {
          ...
          ret = (t) => {
            ...
            let alpha: number = interp_lin(t, t_start + length * 0.8, t_start + length, d.start.color_a, 0.0);
          ...
          let length = 3 / 5;
          ret = (t) => {
            let hue_angle: number = Math.round(interp_lin(t, t_start + length * 2 / 3, t_start + length, currentAngle, targetAngle)) % 360;
            let x: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.x0, d.end.x0);
            let y: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.y0, d.end.y0);
            let w: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.x1 - d.start.x0, d.end.x1 - d.end.x0);
            let h: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.y1 - d.start.y0, d.end.y1 - d.end.y0);
            let sat: number = 0;
            if (t < t_start)
              sat = d.start.color_s;
            else if (t >= t_start && t <= t_start + length / 3)
              sat = (100 - d.start.color_s) / 2 * Math.sin((t - t_start) / length * 3 * 5 * Math.PI - 0.5 * Math.PI) + d.start.color_l + (100 - d.start.color_s) / 2;//interp_hat(t, 1 / 3+1/20, 1/3 + 1 / 6, 2 / 3-1/20, d.start.color_s, 100, d.end.color_s);
            else if (t > t_start + length / 3 && t < t_start + length * 2 / 3)
              sat = 100;
            else
              sat = interp_lin(t, t_start + length * 2 / 3, t_start + length, 100, d.end.color_s)
        ...
        } else // if (d.end.transition == Change.None)
        {
          ...
          let length = 3 / 5;
          ret = (t) => {
            let hue_angle: number = Math.round(interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, currentAngle, targetAngle)) % 360;
            let x: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.x0, d.end.x0);
            let y: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.y0, d.end.y0);
            let w: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.x1 - d.start.x0, d.end.x1 - d.end.x0);
            let h: number = interp_lin(t, t_start + length / 3, t_start + length / 3 * 2, d.start.y1 - d.start.y0, d.end.y1 - d.end.y0);
  ...
  public render(rectangles: RectNode[]) {
    ...
    g.append("foreignObject")
      ...
      .on('mouseover', function (d, i) {
        ...
        tip.style("opacity", 1)
          .html(i.name + " - " + value_smart_print(i.value))
