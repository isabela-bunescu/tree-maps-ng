...
export class DataFetcherService {
  ...
  private decorate_tree(tree_root) {
    ...
    if (tree_root.hasOwnProperty('children')) {
      // node is not leaaf
      let children = tree_root.children.map((c) => {
        return this.decorate_tree(c);
      });
      let values = children.map((el) => {
        return el.total_value;
      });
      let total_value = values.reduce((el) => {
        return el;
      });
      //let total_hash = children.reduce((result :string, current: any) => { return current.hash+result; }, "");
      let hashes = children.map((c) => {
        return c.hash;
      });
      // hashes.concat(Md5.hashStr(tree_root.name));
      hashes.sort();

      let total_hash = hashes.reduce((res, el) => {
        return res + el;
      }, '');
      return {
        name: tree_root.name,
  ...
  public post_index(idx: IndexEntry) {
    return this.http.post<any>(this.base_url + 'data/index', idx);
  }
  ...
  public async get_data(path: string) {
    this.http.get<any>(this.base_url + path).subscribe((dta) => {
      ...
      data = data.map((el) => {
        return { time: el.time, ...decorate_tree(el) };
      });
  ...
  public data_as_tree(): TreeMapNode[] {
    let lst: TreeMapNode[] = [];
    for (let d of this.data) lst.push(TreeConversion(d));
    this.data_tree = lst;
    return lst;
  }
  ...
  public data_as_rectangles(layout: string): RectNode[][] {
    ...
    for (let i = 0; i < rectangles.length; i++)
      rectangles[i] = rectangles[i].sort((a, b) => {
